...
<section className="py-0 px-6 md:px-20 mt-[10px] md:mt-[30px]">
  ...
  <div className="grid grid-cols-2 md:grid-cols-3 gap-4 auto-rows-fr">
    {images.map((src, index) => (
      <div
        key={index}
        className={`overflow-hidden rounded-lg shadow-lg ${index === 0 ? "col-span-2 row-span-2" : "col-span-1 row-span-1"
          }`}
      >
        <img
          src={src}
...
<section className="w-full mt-[10px] md:mt-[30px]">
  ...
  <div className="bg-[#FB6F92] flex justify-center gap-4 py-5">
    <button
      className={`w-36 p-4 border-2 rounded-full transition-all duration-300 ${selectedSession === 'In-Person'
    ...
    <button
      className={`w-36 p-4 border-2 rounded-full transition-all duration-300 ${selectedSession === 'Virtual'
          ? 'bg-white text-[#fb6f92]'
          : 'border-white  text-white hover:bg-[#fb6f92]/10'
        }`}
